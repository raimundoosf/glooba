...
function RelativeTime({ date }: { date: Date }) {
    ...
    useEffect(() => {
        setRelativeTime(formatDistanceToNowStrict(new Date(date), { addSuffix: true, locale: es }));
    }, [date]);
        ...
        <div className="flex items-center space-x-3">
          <Link href={`/profile/${review.author.username}`}>
            <Avatar className="h-9 w-9">
              ...
              <AvatarFallback>{review.author.name?.[0] || review.author.username[0]}</AvatarFallback>
            </Avatar>
          </Link>

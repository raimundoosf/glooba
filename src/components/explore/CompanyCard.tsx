...
export default function CompanyCard({ company }: CompanyCardProps) {
  ...
  const buttonClasses = "flex-shrink-0 px-3 h-8 rounded-full bg-white text-black border border-neutral-300 hover:bg-neutral-100 focus-visible:ring-neutral-400";

  return (
    <Card className="flex flex-col h-full transition-shadow duration-200 hover:shadow-md">
      {/* === MODIFIED CardHeader Alignment === */}
      <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2"> {/* Changed items-start to items-center */}
        ...
        <div className="flex items-center gap-3 flex-shrink min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <Link href={`/profile/${company.username}`} className="min-w-0">
              <CardTitle className="text-lg font-semibold hover:underline break-words"> {/* Allow words to break */}
                {company.name || company.username}
              </CardTitle>
            </Link>
            <CardDescription className="text-sm text-muted-foreground truncate"> {/* Keep username truncated */}
              @{company.username}
            </CardDescription>
          </div>
        </div>
        ...
        <div className="flex-shrink-0">
            {!isOwnProfile && (
                <>
                    {isSignedIn ? (
                        <Button size="icon" onClick={handleFollowToggle} disabled={isPending} className={buttonClasses} aria-label={isFollowingOptimistic ? `Dejar de seguir a ${company.username}` : `Seguir a ${company.username}`} >
                            {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : isFollowingOptimistic ? <UserCheck className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}
                        </Button>
                    ) : (
                        <SignInButton mode="modal">
                            <Button size="icon" className={buttonClasses} aria-label={`Seguir a ${company.username}`} >
      ...
      {/* === End MODIFIED CardHeader Layout === */}
